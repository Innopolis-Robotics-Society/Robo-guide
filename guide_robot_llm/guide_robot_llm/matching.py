...
# Ведущее слово активации ("фирая, стоп") никогда не вырезается ASR из
...
#
# Имя робота -- Фирая. Лексика ОБЯЗАНА совпадать с `activation_phrases`
# wakeword_node (guide_robot_voice/config/voice*.yaml): при переименовании
# на стороне voice без правки здесь `/speech/wakeword` приходит с чужим
# keyword, `_on_wakeword` его отбрасывает и каждый транскрипт уходит в
# «без wakeword, игнор» (живой баг после «feat: new name»). GigaAM пишет имя
# по-разному: «фирая», «фирайя», «фирае», «фираю», «ферая», «фира я»,
# «фи рая» -- отсюда не список, а шаблон: ф[еи] + необязательный пробел +
# ра + необязательный пробел + 1-2 гласных/й. Не «[фв]»: «вера я хочу...»
# резалось бы как обращение.
# Имя вне словаря ASR: «фирая», «ферая», «феррай», «феррари», «фираеи», «фи рая».
_WAKE_NAME = r"ф[еи]\s?р{1,2}\s?а\s?(?:[йяеюи]{1,2}|ри)(?![а-яё])"
# Необязательное обращение перед именем («эй фирая», «слушай фирая»).
_WAKE_PREFIX = r"(?:(?:эй|слушай)[\s,]+)?"

# Ведущий повтор wake-слова с хвостовой пунктуацией ("Фирая, ...", "фирая фирая ...").
...
def has_leading_wake_word(text: str) -> bool:
    """Проверить, что фраза начинается с wake-слова («фирая», «эй фирая, ...»)."""
    return bool(_LEADING_WAKE_RE.match(text))


def idle_turn_allowed(raw: str, *, listen_armed: bool) -> bool:
    """Ход к ЛЛМ только после «фирая» в фразе или окна после /speech/wakeword.
...
def strip_wake_word(text: str) -> str:
    """Срезать ведущее wake-слово («фирая») из финального транскрипта.

    Пустая строка в ответе означает, что реплика состояла ТОЛЬКО из
    wake-слов ("фирая", "Фирая!", "фирая фирая") -- содержания нет, ход к
    ...
    конце фразы ("как тебя зовут фирая") не трогается -- это часть содержания.
    """
    return _LEADING_WAKE_RE.sub("", text, count=1).strip()


def is_wake_keyword(keyword: str) -> bool:
    """`Wakeword.keyword` -- активация (имя робота), а не стоп-слово.

    wakeword_node публикует `Wakeword` и на «стоп»/«хватит»; окно слушания
    dialog_agent открывает только имя. Проверка -- той же регуляркой, что
    режет ведущее обращение из транскрипта: единственный источник лексики.
    """
    return bool(keyword.strip()) and not strip_wake_word(keyword)
...
def match_idle_dismiss(text: str) -> bool:
    ...
    ПОДМНОЖЕСТВО: все токены (кроме ведущего «фирая») обязаны быть
    ...
    """
    tokens = _tokens(strip_wake_word(text))
    if not tokens or len(tokens) > _MAX_TOKENS_FOR_MATCH:
        return False
