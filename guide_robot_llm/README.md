...
```
транскрипт (ведущее wake-слово срезано; голое «Фирая» ход не запускает)
   │
...
`matching.py` (не гейт в ЛЛМ: ход только после «Фирая»).
...
ход без «Фирая»; окно слушания только `_LISTEN_WINDOW_S` после
`/speech/wakeword`),
