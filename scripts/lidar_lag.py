...
стадия "filtered -> /scan" джойнится по ТОЧНОМУ штампу, и это основной путь.
Второй путь — восстановление пары по ближайшим штампам L/R; он нужен для
мерджера, который штампует общим временем (середина развёртки), и включается
сам, если точных совпадений нет. Вариант определяется по данным, настраивать
ничего не надо.
...
def report(bag: dict[str, list[tuple[int, int, bytes]]]) -> None:
    ...
    # Стадия мерджера. dual_laser_merger наследует штамп левого лидара —
    # работает точный джойн. Резервный путь рассчитан на мерджер, который
    # штампует общим временем t_c = (sL + sR)/2 + 50 мс (полуразвёртка): там
    # пара восстанавливается по ближайшим штампам, а ценой стадии считается
    # recv(/scan) - recv(ПОЗДНЕГО из пары), то есть без ожидания партнёра.
    ...
    # Чей штамп несёт /scan. Совпадение с левым = dual_laser_merger, то есть
    # штамп стоит на НАЧАЛЕ развёртки. 0/0 — мерджер с общим временем.
    scan_stamp_set = set(recv_by_stamp["/scan"])
    stamp_at_sweep_start = False
    if scan_stamp_set:
        for side in ("left", "right"):
            ...
            stamp_at_sweep_start |= side == "left" and hits > 0

    # Рассинхрон лидаров: разница штампов спаренных развёрток, спаривание по
    # ближайшему времени. Без компенсации движения эта разница уезжает в /scan
    # как поворот одной половины относительно другой — то есть это прямая цена
    # того, что dual_laser_merger сшивает пару статическим TF.
