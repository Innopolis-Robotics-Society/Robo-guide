...
Orin не выдерживается: замерено в бэге `fp_on_0.db3`, что BT объявил
`compute_path_to_pose` мёртвым на 21.18 c, а сервер опубликовал готовый план
на 521 позу на 21.19 — через 10 мс после таймаута. Дальше по `RoundRobin`
запускались `Spin`, `BackUp` и `Wait` **одновременно** (дерево уже считало
каждый провалившимся), и в `/cmd_vel` параллельно писали три источника: spin
`0.0/1.0`, backup `-0.05/0.0` и хвост `velocity_smoother` `0.66/0.0`. База
получала их вперемешку через 10-20 мс и дёргалась на месте — снаружи это
выглядит как «робот отказывается ехать». `collision_monitor` при этом ни разу
не сработал и честно пропускал всю кашу на привод.
