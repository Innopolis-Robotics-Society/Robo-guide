...
**controller_server / FollowPath (`:147-308`)** — `DWBLocalPlanner`
обёрнут в `nav2_rotation_shim_controller::RotationShimController`
(`:221-222`), чтобы робот сначала доворачивался на месте, а не заходил в
цель по дуге (у DWB нет режима "сначала поворот, потом ход", дуга
дешевле по критикам, но выносит корпус в сектор, не просматриваемый
лидаром на локальном rolling-window костмапе, который не трекает unknown
space). Скорости/ускорения (`:251-265`) подставляются из `robot_params.yaml`
