...
Правила разрешения конфликтов (Say.action):
...
Правила отмены (/speech/cancel_all, CancelAll.msg):
...
# Должно совпадать со строкой CancelAll.msg REASON_ESTOP. Дублирование
...
class Scheduler:
    ...
    def cancel(
        self, scope: Scope | int = Scope.ALL, reason: str = ""
        ...
        if (
            ...
            and self._active.scope.matches(scope)
            and (self._active.interruptible or hard)
        ):
        ...
        for utterance in self._queue:
            if utterance.scope.matches(scope) and (utterance.interruptible or hard):
