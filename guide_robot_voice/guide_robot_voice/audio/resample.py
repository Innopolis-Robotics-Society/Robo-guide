"""Приведение частоты дискретизации синтезатора к частоте устройства.

Зачем отдельный модуль. Голоса Piper для русского работают на 22050 Гц,
Silero -- на 24000 или 48000, а устройство, открытое через hw:, не выполняет
никаких преобразований и требует ровно ту частоту, которую объявило железо.
Типичный USB Audio Class объявляет 48000 и больше ничего.
...
высокочастотные артефакты, о чём модуль честно сообщает.
"""
...
class Resampler:
    ...
    def reset(self) -> None:
        """Сбросить накопленный хвост. Звать при отмене."""
        self._tail = np.zeros(0, dtype=np.int16)
